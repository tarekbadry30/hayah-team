    ...
    <div class="custom-data-table ">
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    <a href="{{route('mobile-sliders.create')}}" class="btn btn-outline-secondary waves-effect waves-light col-sm-3 mx-1"><i class=" fas fa-plus"></i></a>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.toggle-visible', function () {
                ...
                let slider_id = $(this).attr('slider_id');
                $.ajax({
                    url: "{{route('mobile-sliders.toggle')}}",
                    method: 'post',
                    data:{
                        slider_id:slider_id
                    }
        ...
        function setPage(page,itemsPerPage) {
            customDataTable($('#mainTable tbody'),$(".pagination-container"),'{{route('mobile-sliders.dataTable')}}',page,'get',itemsPerPage);
        }
...
visible
                    ...
                    href="{{route('mobile-sliders.index')}}/toggle/${item.id}" title="${item.visible?"{{__('frontend.hide')}}":"{{__('frontend.show')}}" }" slider_id="${item.id}"><i class=" fas fa-eye${item.visible?'':'-slash'}"></i></a>

                    <button
                    class="btn btn-outline-danger delete-btn  waves-effect waves-light"
                    href="{{route('mobile-sliders.index')}}/${item.id}" slider_id="${item.id}" title="{{__('frontend.delete')}}"><i class="fas fa-trash-alt"></i></button>
