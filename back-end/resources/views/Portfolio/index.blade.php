    ...
    <div class="custom-data-table ">
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    <a href="{{route('portfolio.create')}}" class="btn btn-outline-secondary waves-effect waves-light col-sm-3 mx-1"><i class=" fas fa-plus"></i></a>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('submit', '.control-donation', function (e) {
                ...
                $.ajax({
                    ...
                    //$(`.accept-btn[item_id="${form.find('#portfolio_id').val()}"]`).addClass('d-none');
                });
                let tableContent='';;
            });
        ...
        function setPage(page,itemsPerPage) {
            customDataTable($('#mainTable tbody'),$(".pagination-container"),'{{route('portfolio.dataTable')}}',page,'get',itemsPerPage);
        }
            ...
            for(let item of results){
                ...
                tableContent+=`<tr>
                    ...
                    class="btn btn-outline-success waves-effect waves-light toggle-visible"
                     title="${item.visible?"{{__('frontend.hide')}}":"{{__('frontend.show')}}" }" portfolio_id="${item.id}"><i class=" fas fa-eye${item.visible?'':'-slash'}"></i></a>
                    </button>
                    <a class="btn btn-outline-success  waves-effect waves-light"
                    href="{{route('portfolio.index')}}/${item.id}/edit" title="{{__('frontend.edit')}}"><i class="far fa-edit"></i></button>
                    </a>
                    <button
                    class="btn btn-outline-danger delete-btn  waves-effect waves-light"
                    href="{{route('portfolio.index')}}/${item.id}" portfolio_id="${item.id}" title="{{__('frontend.delete')}}"><i class="fas fa-trash-alt"></i></button>
