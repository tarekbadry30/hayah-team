            ...
            <div class="card">
                <div class="card-body">
                    <form novalidate
                          ...
                          action="{{route('donation-helps.update',['donation_help'=>$donationHelp])}}" method="post">
                        @csrf
                        @method('put')
                        <input type="hidden" name="id" value="{{$donationHelp->id}}">
                        <h4 class="header-title">{{__('frontend.editDonationHelp')}} </h4>
                        <!-- Nav tabs -->
                        <div class="row mb-3">
                            <label for="name" class="col-sm-2 col-form-label">{{__('frontend.img')}}</label>
                            <div class="col-sm-10 position-relative">
                                <img src="{{asset($donationHelp->img)}}" class="avatar-lg">
                            </div>
                        </div>
                                ...
                                <div class="tab-pane {{$locale==app()->getLocale()?'active':''}}" id="{{$locale}}" role="tabpanel">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-10 position-relative">
                                            <input class="form-control @error($locale.'.name') parsley-error is-invalid @enderror"
                                                   value="{{old($locale.'.name',$donationHelp->translate($locale)->name)}}"  type="text" placeholder="{{__('frontend.'.$locale.'.name')}}"
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <textarea class="form-control @error($locale.'.desc') parsley-error is-invalid @enderror"
                                                  placeholder="{{__('frontend.'.$locale.'.desc')}}" id="desc" name="{{$locale}}[desc]">{{old($locale.'.desc',$donationHelp->translate($locale)->desc)}}</textarea>
                                            @error($locale.'.desc')
                                            <div class="invalid-tooltip position-static">
                                                {{$message }}
                                            </div>
...
@section('js')
    <script>
        function generateFilters(list,newFilters={
            type_id:'{{old('type_id',$donationHelp->type_id)}}',
            category_id:'{{old('category_id',$donationHelp->category_id)}}',
        }) {
