    ...
    <div class="custom-data-table ">
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                     <a href="{{route('share-ideas.export')}}" class="btn btn-outline-secondary waves-effect waves-light col-sm-2 mx-1"><i class="fas fa-file-export"></i></a>
            ...
            <table id="mainTable" class="mt-3 table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                <thead>
                <tr>
                    <th>{{__('frontend.idea')}}</th>
                    <th>{{__('frontend.target_group')}}</th>
                    <th>{{__('frontend.execution_mechanism')}}</th>
                    <th>{{__('frontend.name')}}</th>
                    <th>{{__('frontend.phone')}}</th>
                    <th>{{__('frontend.money')}}</th>
                    <th>{{__('frontend.timing')}}</th>
...
@section('js')
    <script>
        ...
        function setPage(page,itemsPerPage) {
            customDataTable($('#mainTable tbody'),$(".pagination-container"),'{{route('share-ideas.dataTable')}}',page,'get',itemsPerPage);
        }
           ...
           await $.ajax({
            ...
            for(let item of results){
                ...
                tableContent+=`<tr>
                    ...
                    <td>${item.execution_mechanism}</td>
                    <td>${item.name}</td>
                    <td>${item.phone}</td>
                    <td>${item.money}</td>
                    <td>${item.timing}</td>
                    ...
                    href="{{route('share-ideas.index')}}/${item.id}" title="{{__('frontend.delete')}}"><i class="fas fa-trash-alt"></i></button>
