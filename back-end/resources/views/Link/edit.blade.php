            ...
            <div class="card">
                <div class="card-body">
                    <form novalidate  class="create-form pt-3 {{session()->has('errorsq')?'was-validated':''}}"
                          action="{{route('settings.links.update',['link'=>$link])}}" method="post">
                        @csrf
                        @method('put')
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-10 position-relative">
                                            <input class="form-control @error($locale.'.name') parsley-error is-invalid @enderror"
                                                   value="{{old($locale.'.name',$link->translate($locale)->name)}}"  type="text" placeholder="{{__('frontend.'.$locale.'.name')}}"
                            ...
                            <div class="col-sm-10">
                                <select name="type" class="form-control select2-search-disable2 @error('type') parsley-error is-invalid @enderror ">
                                    ...
                                    @foreach($types as $type)
                                        <option {{old('type',$link->type)==$type?'selected':''}} value="{{$type}}">{{$type}}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                <div class="invalid-tooltip position-static">
