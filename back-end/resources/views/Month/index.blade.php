    ...
    <div class="custom-data-table ">
        ...
        <div class="card-body">
            <h2 class="page-title pt-1">
                {{__('frontend.monthHelpOf')}} <strong>{{$user->name}}</strong>
            </h2>
            <div class="row">
            <!--
                <div class="col-sm-8">
                ...
                <div class="col-sm-4">
                    <a href="{{route('monthly-help.create',['user_id'=>$user->id])}}" class="btn btn-outline-secondary waves-effect waves-light col-sm-3 mx-1"><i class=" fas fa-plus"></i></a>
            ...
            <table id="mainTable" class="mt-3 table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                <thead>
                    <tr>
                        ...
                        <th>{{__('frontend.help_value')}}</th>
                        <th>{{__('frontend.remaining_value')}}</th>
                        <th>{{__('frontend.total_value')}}</th>
                        <th>{{__('frontend.buyTimes')}}</th>
...
@section('js')
        ...
        function setPage(page,itemsPerPage) {
            customDataTable($('#mainTable tbody'),$(".pagination-container"),'{{route('monthly-help.dataTable')}}',page,'get',itemsPerPage);
        }
        async function  customDataTable(tableBody,paginationContainer,url,page=1,method='get',itemsPerPage=15) {
           ...
           await $.ajax({
            ...
            for(let item of results){
                ...
                tableContent+=`<tr>
                    <td>${item.month}</td>
                    <td>${item.help_value}</td>
                    <td>${item.remaining_value}</td>
                    <td>${item.total_value}</td>
                    <td>${item.buy_times_count}</td>
