    ...
    <div class="custom-data-table ">
        ...
        <div class="card-body">
            <h2 class="page-title pt-1">
                {{__('frontend.categoryOptionsOf')}}<strong> {{$category->name}} </strong>
            </h2>
            <div class="row">
                <div class="col-sm-8">
            ...
            <table id="mainTable" class="mt-3 table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                ...
                <tr>
                    ...
                    <th>{{__('frontend.defaultValue')}}</th>
                    <th>{{__('frontend.status')}}</th>
                    <th>{{__('frontend.acceptAnyValue')}}</th>
...
@section('js')
    <script>
        ...
        function setPage(page,itemsPerPage) {
            customDataTable($('#mainTable tbody'),$(".pagination-container"),'{{route('category-option.dataTable')}}?category_id={{$category->id}}',page,'get',itemsPerPage);
        }
